...
# Persian TTS API with PHP Interface - Deployment Script for WSL (Glow-TTS Model)
# This script automates the installation of a Persian TTS API server and web interface on WSL
# Using tts_models/fa/custom/glow-tts model
...
info "Downloading Persian TTS model (glow-tts)..."
sudo -u "$CURRENT_USER" bash << EOF
source "$TTS_SERVER_DIR/venv/bin/activate"
mkdir -p "$TTS_SERVER_DIR/models"
python3 -c "
from TTS.utils.manage import ModelManager
model_name = 'tts_models/fa/custom/glow-tts'
...
# Initialize TTS with Persian model (glow-tts)
try:
    tts = TTS(model_name="tts_models/fa/custom/glow-tts", progress_bar=False)
    print("Successfully loaded Persian TTS model (glow-tts)")
...
[Unit]
Description=Persian TTS API Service (glow-tts)
...
echo
echo "=====================================================
${GREEN}Persian TTS API and Web Interface Installation Complete! (Glow-TTS Model)${NC}
